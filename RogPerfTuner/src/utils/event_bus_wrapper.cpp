...
void EventBusWrapper::emitUsbRemoved(const std::vector<UsbIdentifier>& devices) {
	this->eventBus.emit_event(HARDWARE_SERVICE_USB_REMOVED, {devices});
}

void EventBusWrapper::onBattery(std::function<void(bool)>&& callback) {
	this->eventBus.on_with_data(HARDWARE_SERVICE_ON_BATTERY, [cb = std::move(callback)](CallbackParam data) {
		cb(std::any_cast<bool>(data[0]));
	});
}
void EventBusWrapper::emitBattery(const bool& onBat) {
	this->eventBus.emit_event(HARDWARE_SERVICE_ON_BATTERY, {onBat});
}

void EventBusWrapper::onScheduler(std::function<void(std::optional<std::string>)>&& callback) {
	this->eventBus.on_with_data(PROFILE_SERVICE_ON_SCHEDULER, [cb = std::move(callback)](CallbackParam data) {
		cb(std::any_cast<std::optional<std::string>>(data[0]));
	});
}
void EventBusWrapper::emitScheduler(std::optional<std::string> scheduler) {
	this->eventBus.emit_event(PROFILE_SERVICE_ON_SCHEDULER, {scheduler});
}

void EventBusWrapper::onBootSound(std::function<void(bool)>&& callback) {
	this->eventBus.on_with_data(HARDWARE_SERVICE_BOOT_SOUND_CHANGED, [cb = std::move(callback)](CallbackParam data) {
		cb(std::any_cast<bool>(data[0]));
	});
}
...
void EventBusWrapper::emitUpdateAvailable(std::string version) {
	this->eventBus.emit_event(APPLICATION_SERVICE_UPDATE_AVAILABLE, {version});
}

void EventBusWrapper::onUpdateAvailable(std::function<void(std::string)>&& callback) {
	this->eventBus.on_with_data(APPLICATION_SERVICE_UPDATE_AVAILABLE, [cb = std::move(callback)](CallbackParam data) {
		cb(std::any_cast<std::string>(data[0]));
	});
}

void EventBusWrapper::emitUpdateStart() {
	this->eventBus.emit_event(APPLICATION_SERVICE_UPDATE_START);
}

void EventBusWrapper::onUpdateStart(Callback&& callback) {
	this->eventBus.on_without_data(APPLICATION_SERVICE_UPDATE_START, [cb = std::move(callback)]() {
		cb();
	});
}