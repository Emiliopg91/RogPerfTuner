...
def pack_list(things: list, version: int) -> bytes:
    ...
    :returns: bytes ready to be used
    """
    return bytes(struct.pack("H", len(things))) + b"".join(
        thing.pack(version) for thing in things
    )
...
class LEDData:
    ...
    def pack(self, version: int) -> bytes:
        ...
        :returns: raw data ready to be sent or saved
        """
        return pack_string(self.name) + struct.pack("I", self.value)
...
class ModeData:
    ...
    def validate(self, version: int):
        ...
        try:
            ...
            raise ValueError(
                "Mode validation failed.  Required values invalid or not present"
            ) from e

        try:
            if ModeFlags.HAS_SPEED not in self.flags:
                assert all(
                    (i is None for i in (self.speed_max, self.speed_min, self.speed))
                )
            if ModeFlags.HAS_MODE_SPECIFIC_COLOR not in self.flags:
                assert all(
                    (i is None for i in (self.colors_max, self.colors_min, self.colors))
                )
            ...
            raise ValueError(
                "Mode validation failed.  Values are set that are not supported by this mode"
            ) from e
    ...
    def pack(self, version: int) -> bytes:
        ...
        if version >= 3:
            data += struct.pack(
                "I", self.brightness_min if self.brightness_min is not None else 0
            )
            data += struct.pack(
                "I", self.brightness_max if self.brightness_max is not None else 0
            )
        ...
        if version >= 3:
            data += struct.pack(
                "I", self.brightness if self.brightness is not None else 0
            )
    ...
    def unpack(cls, data: Iterator[int], version: int, index: int = 0) -> ModeData:
        ...
        if (
            ModeFlags.HAS_DIRECTION_HV in flags
...
class SegmentData:
    ...
    def unpack(cls, data: Iterator[int], version: int, *args) -> SegmentData:
        ...
        return cls(name, segment_type, start_idx, leds_count)
...
class ZoneData:
    ...
    def pack(self, version: int) -> bytes:
        ...
        :returns: raw data ready to be sent or saved
        """
        data = pack_string(self.name) + struct.pack(
            "i3I", self.zone_type, self.leds_min, self.leds_max, self.num_leds
        )
        if self.mat_height > 0 and self.mat_width > 0:  # type: ignore
            ...
            data += struct.pack(
                f"HII{len(flat)}I", len(flat), self.mat_height, self.mat_width, *flat
            )
        else:
...
class MetaData:
    ...
    def unpack(cls, data: Iterator[int], version: int, *args) -> MetaData:
        ...
        return cls(vendor, description, fw_version, serial, location)
...
class ControllerData:
    ...
    def pack(self, version: int) -> bytes:
        ...
        buff = (
            ...
            + b"".join(
                mode.pack(version)[struct.calcsize("Ii") :] for mode in self.modes
            )
    ...
    def unpack(cls, raw_data: bytes, version: int, start: int = 0) -> ControllerData:
        ...
        # print(*colors, sep="\n\t")
        # print("---------------------------------")
        return cls(name, metadata, device_type, leds, zones, modes, colors, active_mode)
...
class Plugin:
    ...
    def unpack(cls, data: Iterator[int], version: int, *args) -> Plugin:
        ...
        return cls(name, description, version_str, id, sdk_version)
...
class RGBContainer(RGBObject):
    ...
    def show(self, fast: bool = False, force: bool = False):
        ...
        if len(self.colors) > len(self._colors):
            raise ValueError(
                f"`self.colors` is longer than expected length `{len(self._colors)}`"
            )
        elif len(self.colors) < len(self._colors):
            raise ValueError(
                f"`self.colors` is shorter than expected length `{len(self._colors)}`"
            )
        changed = [
            (i, color)
