...
class Logger:
    ...
    @staticmethod
    def set_config_map(cfg_map: dict[str, str]):
        """Set logger config map with levels associated"""
        Logger.config_map = cfg_map

    @staticmethod
    def initialize(log_file, log_folder, log_old_folder):
        """Initialize logger class (files, levels...)"""
        ...
        for lf in [
            ...
            if file != "runner.log" and file != "OpenRGB.log" and file.endswith(".log")
        ]:
            shutil.move(os.path.join(log_folder, lf), log_old_folder)

        old_files = [
            os.path.join(log_old_folder, archivo) for archivo in os.listdir(log_old_folder) if archivo.endswith(".log")
        ]
