...
class Toaster : public Singleton<Toaster> {
  public:
	Toaster(const std::optional<std::string>& appName = std::nullopt, const std::optional<std::string>& icon = std::nullopt);
	void showToast(const std::string& message, bool can_be_hidden = true);
