...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), _logger(new Logger()) {
	...
	QGroupBox* performanceGroup =
		new QGroupBox(QString::fromStdString(Translator::getInstance().translate("performance")));
	...
	_gameProfileButton =
		new QPushButton(QString::fromStdString(Translator::getInstance().translate("label.game.configure")));
	...
	for (RgbBrightness b : brightnesses) {
		_brightnessDropdown->addItem(
			QString::fromStdString(Translator::getInstance().translate("label.brightness." + b.toName())), b.toInt());
	}
	_brightnessDropdown->setCurrentIndex(
		_brightnessDropdown->findData(OpenRgbService::getInstance().getCurrentBrightness().toInt()));
	connect(_brightnessDropdown, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
			&MainWindow::onBrightnessChange);
	...
	connect(_thresholdDropdown, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
			&MainWindow::onBatteryLimitChanged);
