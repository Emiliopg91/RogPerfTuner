...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), _logger(new Logger()) {
	...
	for (PerformanceProfile item : items) {
		_profileDropdown->addItem(QString::fromStdString(Translator::getInstance().translate("label.profile." + item.toName())), item.toInt());
	}
	...
	performanceLayout->addRow(new QLabel(QString::fromStdString(Translator::getInstance().translate("profile") + ":")), _profileDropdown);
	...
	performanceLayout->addRow(new QLabel(QString::fromStdString(Translator::getInstance().translate("games") + ":")), _gameProfileButton);
	...
	auraLayout->addRow(new QLabel(QString::fromStdString(Translator::getInstance().translate("effect") + ":")), _effectDropdown);
	...
	for (RgbBrightness b : brightnesses) {
		_brightnessDropdown->addItem(QString::fromStdString(Translator::getInstance().translate("label.brightness." + b.toName())), b.toInt());
	}
	...
	auraLayout->addRow(new QLabel(QString::fromStdString(Translator::getInstance().translate("brightness") + ":")), _brightnessDropdown);
	...
	settingsLayout->addRow(new QLabel(QString::fromStdString(Translator::getInstance().translate("charge.threshold") + ":")), _thresholdDropdown);
	...
	settingsLayout->addRow(_autostart, new QLabel(QString::fromStdString(Translator::getInstance().translate("autostart"))));
	...
	EventBus::getInstance().on_with_data(Events::STEAM_SERVICE_GAME_EVENT, [this](CallbackParam data) { onGameEvent(std::any_cast<int>(data[0])); });
}
