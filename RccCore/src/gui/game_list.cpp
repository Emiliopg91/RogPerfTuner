...
GameList::GameList(QWidget *parent, bool manage_parent)
    : QDialog(parent), manageParent(manage_parent), parentWidget(parent)
{
    if (!INSTANCE)
        INSTANCE = this;
    ...
    for (unsigned int appid : appIds)
    {
        ...
        QTableWidgetItem *item = new QTableWidgetItem(
            QString::fromStdString(gameCfg[std::to_string(appid)].name +
                                   (isRunning ? " (" + Translator::getInstance().translate("running") + "...)" : "")));
        ...
        for (const auto &[key, val] : gpus)
        {
            ...
            if (SteamService::getInstance().getPreferedGpu(appid).has_value() && key == SteamService::getInstance().getPreferedGpu(appid).value().toString())
                gpuIndex = i + 1;
            i++;
        }
        ...
        connect(gpuCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), [=](int)
                { 
                    ...
                    SteamService::getInstance().setPreferedGpu(
                                                    appid, 
                                                    brand
                                                ); });
        ...
        connect(sdCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), [=](int)
                { SteamService::getInstance().setSteamDeck(appid, sdCombo->currentData().toBool()); });
        ...
        if (SteamService::getInstance().metricsEnabled())
        {
            ...
            {
                metricsCombo->addItem(QString::fromStdString(Translator::getInstance().translate("label.level." + std::to_string(level.toInt()))), level.toInt());
            ...
            connect(metricsCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), [=](int)
                    { SteamService::getInstance().setMetricsLevel(appid, MangoHudLevel::fromInt(metricsCombo->currentData().toInt())); });
        ...
        {
            syncCombo->addItem(QString::fromStdString(Translator::getInstance().translate("label.winesync." + opt.toString())), QString::fromStdString(opt.toString()));
        ...
        connect(syncCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), [=](int)
                { SteamService::getInstance().setWineSync(appid, syncCombo->currentData().toString().toStdString()); });
        ...
        connect(envInput, &QLineEdit::textChanged, [=](const QString &text)
                { SteamService::getInstance().setEnvironment(appid, text.toStdString()); });
        ...
        connect(paramsInput, &QLineEdit::textChanged, [=](const QString &text)
                { SteamService::getInstance().setParameters(appid, text.toStdString()); });
