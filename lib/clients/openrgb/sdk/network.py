...
class NetworkClient:
    ...
    def start_connection(self):
        ...
        self.send_header(
            0, utils.PacketType.REQUEST_PROTOCOL_VERSION, struct.calcsize("I")
        )
        ...
        if buff[:4] == [b"O", b"R", b"G", b"B"]:
            ...
            if packet_type == utils.PacketType.REQUEST_CONTROLLER_COUNT:
                ...
                try:
                    ...
                    self._protocol_version = min(
                        self.max_protocol_version, self._protocol_version
                    )
                except utils.CONNECTION_ERRORS as e:
                    self.stop_connection()
                    raise utils.OpenRGBDisconnected() from e
                ...
                self.callback(
                    device_id,
                    packet_type,
                    utils.parse_list(utils.Profile, idata, self._protocol_version),
                )
                ...
                self.callback(
                    device_id,
                    packet_type,
                    utils.parse_list(utils.Plugin, idata, self._protocol_version),
                )
    ...
    def requestDeviceData(self, device: int):
        ...
        if not self.connected:
            raise utils.OpenRGBDisconnected()
        self.send_header(
            device, utils.PacketType.REQUEST_CONTROLLER_DATA, struct.calcsize("I")
        )
        self.send_data(struct.pack("I", self._protocol_version), False)
        self.read()
    ...
    def send_header(
        ...
        try:
            data = struct.pack(
                "ccccIII", b"O", b"R", b"G", b"B", device_id, packet_type, packet_size
            )
            ...
            if (
                ...
                and packet_type
                not in (
                    utils.PacketType.REQUEST_CONTROLLER_COUNT,
    ...
    def check_version(self, packet_type: utils.PacketType):
        ...
        if self._protocol_version < 2 and packet_type in (
            ...
            and packet_type == utils.PacketType.RGBCONTROLLER_SAVEMODE
        ):
            raise utils.SDKVersionError(
