...
class Segment(utils.RGBContainer):
    ...
    def _update(self, data: utils.SegmentData):
        ...
        self.leds = self.parent_zone.leds[
            data.start_idx : data.start_idx + data.leds_count
        ]
...
class Zone(utils.RGBContainer):
    ...
    def set_color(self, color: utils.RGBColor, fast: bool = False):
        ...
        buff = struct.pack("iH", self.id, len(self.leds)) + (color.pack()) * len(
            self.leds
        )
    ...
    def set_colors(self, colors: list[utils.RGBColor], fast: bool = False):
        ...
        buff = struct.pack("iH", self.id, len(self.leds)) + b"".join(
            (color.pack() for color in colors)
        )
        buff = struct.pack("I", len(buff) + struct.calcsize("I")) + buff
        self.comms.send_data(buff)
        if not fast:
...
class Device(utils.RGBContainer):
    ...
    def __init__(
        self, data: utils.ControllerData, device_id: int, network_client: NetworkClient
    ):
    ...
    def _update(self, data: utils.ControllerData):
        ...
        for x in range(len(data.leds)):
            if self.leds[x] is None:
                self.leds[x] = LED(
                    data.leds[x], data.colors[x], x, self.device_id, self.comms
                )
            else:
                self.leds[x]._update(data.leds[x], data.colors[x])
    ...
    def _set_device_colors(self, colors: list[utils.RGBColor], fast: bool = False):
        ...
        buff = struct.pack("H", len(self.leds)) + b"".join(
            (color.pack() for color in colors)
        )
        buff = struct.pack("I", len(buff) + struct.calcsize("I")) + buff
        self.comms.send_data(buff)
    ...
    def _set_mode_colors(self, colors: list[utils.RGBColor]):
        ...
        assert active_mode.colors is not None
        assert (
            active_mode.colors_min
            <= len(colors)  # type: ignore
            <= active_mode.colors_max
        )
    ...
    def set_mode(self, mode: Union[str, int, utils.ModeData], save: bool = False):
        ...
        if isinstance(mode, str):
            try:
                ...
                raise ValueError(
                    f"Mode `{mode}` not found for device `{self.name}`"
                ) from e
        ...
        self.comms.send_header(
            self.id, utils.PacketType.RGBCONTROLLER_UPDATEMODE, len(data)
        )
        self.comms.send_data(data)
        if save:
            self.comms.send_header(
                self.id, utils.PacketType.RGBCONTROLLER_SAVEMODE, len(data)
            )
            self.comms.send_data(data)
        self.update()
...
class OpenRGBClient(utils.RGBObject):
    ...
    def __repr__(self):
        return (
            f"OpenRGBClient(address={self.address}, port={self.port}, name={self.name})"
        )
    ...
    def get_devices_by_name(self, name: str, exact: bool = True) -> list[Device]:
        ...
        return [
            device for device in self.devices if name.lower() in device.name.lower()
        ]

    def load_profile(
        self,
        name: Union[str, int, utils.Profile],
        local: bool = False,
        directory: str = "",
    ):
        """
                ...
                for device in self.devices:
                    for new_controller in controllers:
                        if (
                            ...
                            and new_controller.metadata.description
