...
class OpenRgbService:
    def __init__(self):
        ...
        if configuration.openRgb.lastEffect and configuration.openRgb.lastEffect in configuration.openRgb.effects:
            self.color = configuration.openRgb.effects[configuration.openRgb.lastEffect].color 

        self.brightness = RgbBrightness.MEDIUM
        if configuration.openRgb.lastEffect and configuration.openRgb.lastEffect in configuration.openRgb.effects:
            self.brightness = RgbBrightness(configuration.openRgb.effects[configuration.openRgb.lastEffect].brightness)
    ...
    def _apply_aura(self, effect:str, brightness:RgbBrightness, color: Optional[str] = None, force = False) -> None:
        ...
        if (self.effect != effect or self.brightness != brightness or self.color != color) or force:
            ...
            self.color = color
            event_bus.emit("OpenRgbService.aura_changed", {effect: effect, brightness: brightness, color: color})
            configuration.config.openRgb.lastEffect = effect
            configuration.config.openRgb.effects[effect] = Effect(brightness.value, color)
