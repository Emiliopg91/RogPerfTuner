...
from PyQt5.QtWidgets import (
    QApplication, QVBoxLayout, QFormLayout, QLabel, QComboBox, QGroupBox, QMainWindow, QWidget, QPushButton, QColorDialog
)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.profile_labels= [translator.translate(f"label.profile.{profile.name}") for profile in ThermalThrottleProfile]
        ...
        self.brightness_labels = [translator.translate(f"label.brightness.{brightness.name}") for brightness in RgbBrightness]
        ...
        self.brightness_dropdown.setCurrentIndex(self.brightness_items.index(open_rgb_service.brightness))
        ...
        self.color_button.setStyleSheet(f"background-color: {self.current_color if self.current_color is not None else "#00000000"};")
        self.color_button.clicked.connect(self.pick_color)
        self.color_button.setDisabled(not open_rgb_service.supports_color(open_rgb_service.effect))
        aura_layout.addRow(QLabel(f"{translator.translate('color')}:"), self.color_button)
        ...
        settings_layout.addRow(QLabel(f"{translator.translate("charge.threshold")}:",), self.threshold_dropdown)
    ...
    def center_window_on_current_screen(self):
        ...
        if screen is not None:
            ...
            y = int((screen_geometry.height() - self.height()) / 2) + screen_geometry.y()
            self.move(x, y)
    ...
    def set_aura_state(self, value):
        ...
        self.brightness_dropdown.setCurrentIndex(self.brightness_items.index(brightness))
