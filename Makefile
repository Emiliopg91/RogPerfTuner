...
config:
	...
	CCACHE_NOHASHDIR=1 cmake -B build -G Ninja -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -S . -DCMAKE_BUILD_TYPE=$(BUILD_TYPE) -DCMAKE_EXPORT_COMPILE_COMMANDS=ON $${DEV_MODE:+-DDEV_MODE=1} 
...
build_openrgb:
	...
	@if [ ! -d "build/assets/OpenRGB" ]; then \
		...
		echo "#######################################################################" && \
		cd submodules/OpenRGB && env CCACHE_NOHASHDIR=1 CMAKE_CXX_COMPILER_LAUNCHER=ccache ./build.sh && \
