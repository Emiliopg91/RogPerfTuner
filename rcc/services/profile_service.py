...
class ProfileService:
    ...
    def set_performance_profile(  # pylint: disable=R0914
        ...
        with self._lock:
            ...
            if self._performance_profile != profile or force:
                ...
                cpu_governor = profile.battery_governor if UPOWER_CLIENT.on_battery else profile.ac_governor
                boost_enabled = profile.battery_boost if UPOWER_CLIENT.on_battery else profile.ac_boost
                try:
                    self._logger.info(
    ...
    def restore_profile(self):
        ...
        if UPOWER_CLIENT.on_battery:
            ...
            self._logger.add_tab()
            self._on_ac_battery_change(UPOWER_CLIENT.on_battery, True)
            self._logger.rem_tab()
        else:
