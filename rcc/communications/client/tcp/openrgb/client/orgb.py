...
import struct
import platform
from rcc.communications.client.tcp.openrgb.client.utils import *
from rcc.communications.client.tcp.openrgb.client.network import NetworkClient
from rcc.communications.client.tcp.openrgb.client.plugins import create_plugin, ORGBPlugin
...
class LED(RGBObject):
    ...
    def __init__(self, data: LEDData, color: RGBColor, led_id: int, device_id: int, network_client: NetworkClient):
        self.id = led_id
        self.device_id = device_id
        self.comms = network_client
        self._update(data, color)

    def _update(self, data: LEDData, color: RGBColor):
        self.name = data.name
        self.colors = [color]
    ...
    def set_color(self, color: RGBColor, fast: bool = False):
        ...
        """
        self.comms.send_header(self.device_id, PacketType.RGBCONTROLLER_UPDATESINGLELED, struct.calcsize("i3bx"))
...
class Segment(RGBContainer):
    """
    A class to represent a segment
    """

    def __init__(self, data: SegmentData, id: int, parent: Zone):
        self.leds = [None for _ in range(data.leds_count)]
        self.id = id
        self.parent_zone = parent
        self._update(data)

    def _update(self, data: SegmentData):
        self.name = data.name
    ...
    def set_colors(self, colors: list[RGBColor], fast: bool = False):
        new_colors = (
            self.parent_zone.colors[: self.start_idx]
            + colors
            + self.parent_zone.colors[self.start_idx + self.leds_count :]
        )
        self.parent_zone.set_colors(new_colors, fast)


class Zone(RGBContainer):
    """
    A class to represent a zone
    """

    def __init__(self, data: ZoneData, zone_id: int, device_id: int, network_client: NetworkClient):
    ...
    def _update(self, data: ZoneData):
        self.name = data.name
    ...
    def resize(self, size: int):
        ...
        """
        self.comms.send_header(self.device_id, PacketType.RGBCONTROLLER_RESIZEZONE, struct.calcsize("ii"))
        self.comms.send_data(struct.pack("ii", self.id, size))
        self.update()


class Device(RGBContainer):
    """
    ...
    def _update(self, data: ControllerData):
        self.name = data.name
    ...
    def _set_device_color(self, color: RGBColor, fast: bool = False):
        """
        ...
        if self.enabled:
            self.comms.send_header(
                self.id,
                PacketType.RGBCONTROLLER_UPDATELEDS,
    ...
    def _set_device_colors(self, colors: list[RGBColor], fast: bool = False):
        """
    ...
    def _set_mode_color(self, color: RGBColor):
        """
        ...
        if self.enabled:
            ...
            assert active_mode.color_mode == ModeColors.MODE_SPECIFIC
    ...
    def _set_mode_colors(self, colors: list[RGBColor]):
        """
        Sets the mode-specific color, if possible
    ...
    def set_mode(self, mode: Union[str, int, ModeData], save: bool = False):
        """
        ...
        if self.enabled:
            ...
            elif isinstance(mode, ModeData):
                pass
            else:
                raise TypeError()
            data = mode.pack(self.comms._protocol_version)  # type: ignore
            self.comms.send_header(self.id, PacketType.RGBCONTROLLER_UPDATEMODE, len(data))
            self.comms.send_data(data)
            if save:
                self.comms.send_header(self.id, PacketType.RGBCONTROLLER_SAVEMODE, len(data))
                self.comms.send_data(data)
            self.update()
    ...
    def set_custom_mode(self):
        ...
        """
        if self.enabled:
            self.comms.send_header(self.id, PacketType.RGBCONTROLLER_SETCUSTOMMODE, 0)
            self.update()
...
class OpenRGBClient(RGBObject):
    ...
    def _callback(self, device: int, type: int, data: Any):
        if type == PacketType.REQUEST_CONTROLLER_COUNT:
        ...
        elif type == PacketType.REQUEST_CONTROLLER_DATA:
            try:
        ...
        elif type == PacketType.DEVICE_LIST_UPDATED:
            self.device_num = 0
            self.comms.requestDeviceNum()
        elif type == PacketType.REQUEST_PROFILE_LIST:
            self.profiles = data
        elif type == PacketType.REQUEST_PLUGIN_LIST:
            for plugin in data:
                if all(plugin.id != existing.id for existing in self.plugins):
                    self.plugins.append(create_plugin(plugin, self.comms))
        elif type == PacketType.PLUGIN_SPECIFIC:
    ...
    def get_devices_by_type(self, type: DeviceType) -> list[Device]:
        """
    ...
    def load_profile(self, name: Union[str, int, Profile], local: bool = False, directory: str = ""):
        """
        ...
        if local:
            ...
            with open(f"{directory}/{name}.orp", "rb") as f:
                controllers = LocalProfile.unpack(f).controllers
                pairs = []
                for device in self.devices:
                    for new_controller in controllers:
                        if (
                            new_controller.name == device.name
            ...
            elif isinstance(name, Profile):
                pass
            else:
                raise TypeError()
            raw_name = name.pack()  # type: ignore
            self.comms.send_header(0, PacketType.REQUEST_LOAD_PROFILE, len(raw_name))
            self.comms.send_data(raw_name)

    def save_profile(self, name: Union[str, int, Profile], local: bool = False, directory: str = ""):
        """
        ...
        if local:
            ...
            with open(f'{directory.rstrip("/")}/{name}.orp', "wb") as f:
                f.write(LocalProfile([dev.data for dev in self.devices]).pack())
        else:
            if isinstance(name, str):
                try:
                    name = next(p for p in self.profiles if p.name.lower() == name.lower())
                except StopIteration:
                    name = Profile(name)  # type: ignore
            elif isinstance(name, int):
                name = self.profiles[name]
            elif isinstance(name, Profile):
                pass
                ...
                raise TypeError()
            raw_name = name.pack()  # type: ignore
            self.comms.send_header(0, PacketType.REQUEST_SAVE_PROFILE, len(raw_name))
            self.comms.send_data(raw_name)
            self.update_profiles()

    def delete_profile(self, name: Union[str, int, Profile]):
        """
        ...
        if isinstance(name, str):
            ...
            raise TypeError()
        raw_name = name.pack()  # type: ignore
        self.comms.send_header(0, PacketType.REQUEST_DELETE_PROFILE, len(raw_name))
        self.comms.send_data(raw_name)
        self.update_profiles()
