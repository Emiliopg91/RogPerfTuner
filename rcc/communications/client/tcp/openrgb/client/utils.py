...
class RGBColor:
    ...
    def transition_to(self, dest: RGBColor, steps: int) -> list[RGBColor]:
        transition = []

        start_h, start_s, start_v = self.to_hsv()
        dest_h, dest_s, dest_v = dest.to_hsv()

        for i in range(steps - 1):
            factor = (i + 1) / steps

            h = start_h + (dest_h - start_h) * factor
            s = start_s + (dest_s - start_s) * factor
            v = start_v + (dest_v - start_v) * factor

            r, g, b = colorsys.hsv_to_rgb(h, s, v)

            transition.append(RGBColor(round(r * 255), round(g * 255), round(b * 255)))

        transition.append(dest)
...
class ZoneData:
    ...
    def pack(self, version: int) -> bytes:
        ...
        :returns: raw data ready to be sent or saved
        """
        data = pack_string(self.name) + struct.pack("i3I", self.zone_type, self.leds_min, self.leds_max, self.num_leds)
        if self.mat_height > 0 and self.mat_width > 0:  # type: ignore
            ...
            data += struct.pack(f"HII{len(flat)}I", len(flat), self.mat_height, self.mat_width, *flat)
        else:
    ...
    def unpack(cls, data: Iterator[int], version: int, *args) -> ZoneData:
        ...
        if version >= 4:
            ...
            segments = None
        return cls(name, zone_type, leds_min, leds_max, num_leds, height, width, matrix, segments)
